...
class Spyro2Item(Item):
    ...
    def get_name_to_id() -> dict:
        ...
        return {
            item_data.name: (
                base_id + item_data.s2_code if item_data.s2_code is not None else None
            )
            for item_data in _all_items
        }


key_item_names = {"Summer Forest Talisman", "Autumn Plains Talisman", "Orb"}


_all_items = [
    Spyro2ItemData(row[0], row[1], row[2])
    for row in [
...
def BuildItemPool(world, count, options, locked_levels):
    ...
    if (
        ...
        and world.options.open_world_ability_and_warp_unlocks.value
    ):
    ...
    if options.moneybags_settings.value == MoneybagsOptions.MONEYBAGSSANITY:
        ...
        if (
            not world.options.enable_open_world.value
            or not world.options.open_world_ability_and_warp_unlocks.value
        ):
    ...
    if options.enable_gemsanity.value == GemsanityOptions.PARTIAL:
        ...
        remaining_count -= 168
    elif options.enable_gemsanity.value in [
    ...
    if options.enable_progressive_sparx_health in [
        SparxUpgradeOptions.BLUE,
        SparxUpgradeOptions.GREEN,
        SparxUpgradeOptions.SPARXLESS,
    ]:
    ...
    if remaining_count < 0:
        raise OptionError(
            f"The options you have selected have {remaining_count * -1} more items than locations. You must choose options that add at least this many more locations."
        )
    ...
    for item in _all_items:
        if item.name == "Extra Life" and options.enable_filler_extra_lives:
            ...
            item.name == "Destructive Spyro" and options.enable_destructive_spyro_filler
        ):
            for i in range(0, 6):
                allowed_misc_items.append(item)
            ...
            item.name == "Big Head Mode" or item.name == "Flat Spyro Mode"
        ) and options.enable_filler_big_head_mode:
            for i in range(0, 3):
                allowed_misc_items.append(item)
            ...
            allowed_trap_items.append(item)
        elif item.name == "Normal Spyro" and (
            options.enable_filler_color_change
            or options.enable_filler_big_head_mode
            or len(allowed_misc_items) == 0
        ):
            for i in range(0, 4):
                allowed_misc_items.append(item)
    ...
    if (
        remaining_count > 0
        and options.trap_filler_percent.value > 0
        and len(allowed_trap_items) == 0
    ):
        raise OptionError(
            f"Trap percentage is set to {options.trap_filler_percent.value}, but none have been turned on."
        )
    if (
        remaining_count > 0
        and options.trap_filler_percent.value < 100
        and len(allowed_misc_items) == 0
    ):
        raise OptionError(
            f"{100 - options.trap_filler_percent.value} percent of filler items are meant to be non-traps, but no non-trap items have been turned on."
        )
