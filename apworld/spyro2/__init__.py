...
class Spyro2World(World):
    ...
    glitches_item_name: str = (
        "Glitched Item"  # UT Glitched Logic Support, Not implemented yet.
    )
    ...
    def generate_early(self):
        ...
        if self.options.goal.value in [
            GoalOptions.ALL_SKILLPOINTS,
            GoalOptions.EPILOGUE,
        ]:
        ...
        if self.options.enable_gemsanity.value == GemsanityOptions.PARTIAL:
            ...
            if is_ut:
                ...
                self.chosen_gem_locations = self.multiworld.random.sample(
                    all_gem_locations, k=200
                )
        if self.options.enable_gemsanity.value in [
            GemsanityOptions.FULL,
            GemsanityOptions.FULL_GLOBAL,
        ]:
            if not self.settings.allow_full_gemsanity and self.multiworld.players > 1:
                raise OptionError(
                    f"Spyro 2: Player {self.player_name} has gemsanity set to full, which adds 2546 progression "
        ...
        if hasattr(self.multiworld, "re_gen_passthrough"):
            self.key_locked_levels = self.multiworld.re_gen_passthrough["Spyro 2"][
                "key_locked_levels"
            ]
        else:
            ...
            self.key_locked_levels = self.multiworld.random.sample(
                possible_locked_levels, k=22 - self.options.level_unlocks.value
            )
    ...
    def create_regions(self):
        ...
        regions.update(
            {
                region_name: self.create_region(
                    region_name, location_tables[region_name]
                )
                for region_name in (self.all_levels + ["Inventory"])
            }
        )
    ...
    def create_region(self, region_name, location_table) -> Region:
        ...
        for location in location_table:
            if (
                location.category in self.enabled_location_categories
                and location.category
                not in [
                    Spyro2LocationCategory.EVENT,
                    Spyro2LocationCategory.TOTAL_GEM,
                ...
                location.category in self.enabled_location_categories
                and location.category == Spyro2LocationCategory.GEM
                and (
                    len(self.chosen_gem_locations) == 0
                    or location.name in self.chosen_gem_locations
                )
            ):
                new_location = Spyro2Location(
                    self.player,
                ...
                location.category in self.enabled_location_categories
                and location.category == Spyro2LocationCategory.TOTAL_GEM
            ):
    ...
    def create_item(self, name: str) -> Item:
        ...
        if (
            name in key_item_names
            or name == "Glitched Item"
            or item_dictionary[name].category
            in [
                Spyro2ItemCategory.LEVEL_UNLOCK,
                Spyro2ItemCategory.TALISMAN,
                Spyro2ItemCategory.ORB,
                Spyro2ItemCategory.EVENT,
                Spyro2ItemCategory.MONEYBAGS,
                Spyro2ItemCategory.SKILLPOINT_GOAL,
                Spyro2ItemCategory.TOKEN,
                Spyro2ItemCategory.GEM,
                Spyro2ItemCategory.GEMSANITY_PARTIAL,
            ]
            or self.options.enable_progressive_sparx_logic.value
            and name == "Progressive Sparx Health Upgrade"
        ):
            item_classification = ItemClassification.progression
        elif (
            item_dictionary[name].category in useful_categories
            or not self.options.enable_progressive_sparx_logic.value
            and name == "Progressive Sparx Health Upgrade"
            or name in ["Double Jump Ability", "Permanent Fireball Ability"]
        ):
    ...
    def set_rules(self) -> None:
        def is_boss_defeated(self, boss, state):
            if (
                self.options.enable_open_world
                and self.options.open_world_ability_and_warp_unlocks
                and boss in ["Crush", "Gulp"]
            ):
                return True
        ...
        def can_swim(self, state):
            return state.has(
                "Moneybags Unlock - Swim", self.player
            ) or is_boss_defeated(self, "Ripto", state)

        def can_climb(self, state):
            return state.has(
                "Moneybags Unlock - Climb", self.player
            ) or is_boss_defeated(self, "Ripto", state)

        def can_headbash(self, state):
            return state.has(
                "Moneybags Unlock - Headbash", self.player
            ) or is_boss_defeated(self, "Ripto", state)
        ...
        def can_pass_autumn_door(self, state):
            return can_reach_autumn_second_half(self, state) and state.has(
                "Orb", self.player, 8
            )
        ...
        def get_gems_accessible_in_level(self, level, state):
            ...
            if level == "Glimmer":
                ...
                if can_swim(self, state):
                    ...
                    gems += 221
                    if state.has(
                        "Moneybags Unlock - Door to Aquaria Towers", self.player
                    ) or is_boss_defeated(self, "Ripto", state):
                        gems += 14
                    if can_climb(self, state):
                        gems += 10
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Idol Springs Unlock", self.player)
                ):
                    return 0
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Colossus Unlock", self.player)
                ):
                    return 0
                return 400
            elif level == "Hurricos":
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Hurricos Unlock", self.player)
                ):
                    return 0
                return 400
            elif level == "Aquaria Towers":
                if (
                    not can_reach_summer_second_half(self, state)
                    or not (
                        state.has(
                            "Moneybags Unlock - Door to Aquaria Towers", self.player
                        )
                    ...
                    or (
                        self.options.enable_progressive_sparx_logic.value
                        and not has_sparx_health(self, 1, state)
                    )
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Aquaria Towers Unlock", self.player)
                ):
                    return 0
                # TODO: Allow for getting in without swim as a trick.
                gems = 127
                if state.has(
                    "Moneybags Unlock - Aquaria Towers Submarine", self.player
                ) or is_boss_defeated(self, "Ripto", state):
                    gems += 273
                return gems
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Sunny Beach Unlock", self.player)
                ):
                    return 0
                ...
                if not can_reach_summer_second_half(self, state) or not state.has(
                    "Orb", self.player, 3
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Ocean Speedway Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if not (
                    self.options.enable_open_world
                    and self.options.open_world_ability_and_warp_unlocks
                ) and not is_boss_defeated(self, "Crush", state):
                    return 0
                gems = 118
                ...
                if can_pass_autumn_door(self, state):
                    gems += 202
                    if state.has(
                        "Moneybags Unlock - Shady Oasis Portal", self.player
                    ) or is_boss_defeated(self, "Ripto", state):
                        gems += 7
                return gems
                ...
                if not is_boss_defeated(self, "Crush", state) or (
                    self.options.enable_progressive_sparx_logic.value
                    and not has_sparx_health(self, 2, state)
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Skelos Badlands Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Crystal Glacier Unlock", self.player)
                ):
                    return 0
                gems = 245
                if state.has(
                    "Moneybags Unlock - Crystal Glacier Bridge", self.player
                ) or is_boss_defeated(self, "Ripto", state):
                    gems += 155
                return gems
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Breeze Harbor Unlock", self.player)
                ):
                    return 0
                return 400
            elif level == "Zephyr":
                if not is_boss_defeated(self, "Crush", state) or not (
                    state.has("Moneybags Unlock - Zephyr Portal", self.player)
                    or is_boss_defeated(self, "Ripto", state)
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Zephyr Unlock", self.player)
                ):
                    return 0
                gems = 284
                if can_climb(self, state):
                ...
                if not is_boss_defeated(self, "Crush", state) or not can_reach_metro(
                    self, state
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Metro Speedway Unlock", self.player)
                ):
                    return 0
                return 400
            elif level == "Scorch":
                if not is_boss_defeated(
                    self, "Crush", state
                ) or not can_reach_autumn_second_half(self, state):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Scorch Unlock", self.player)
                ):
                    return 0
                return 400
            elif level == "Shady Oasis":
                if (
                    not is_boss_defeated(self, "Crush", state)
                    or not can_pass_autumn_door(self, state)
                    or not (
                        state.has("Moneybags Unlock - Shady Oasis Portal", self.player)
                        or is_boss_defeated(self, "Ripto", state)
                    )
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Shady Oasis Unlock", self.player)
                ):
                    return 0
                gems = 380
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Magma Cone Unlock", self.player)
                ):
                    return 0
                gems = 295
                if state.has(
                    "Moneybags Unlock - Magma Cone Elevator", self.player
                ) or is_boss_defeated(self, "Ripto", state):
                    gems += 105
                return gems
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Fracture Hills Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if (
                    ...
                    or not can_pass_autumn_door(self, state)
                    or not (
                        state.has("Moneybags Unlock - Icy Speedway Portal", self.player)
                        or is_boss_defeated(self, "Ripto", state)
                    )
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Icy Speedway Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if not (
                    self.options.enable_open_world
                    and self.options.open_world_ability_and_warp_unlocks
                ) and not is_boss_defeated(self, "Gulp", state):
                    return 0
                gems = 139
                if can_reach_winter_second_half(self, state):
                    gems += 254
                    if state.has(
                        "Orb", self.player, self.options.ripto_door_orbs.value
                    ):
                        gems += 7
                return gems
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Mystic Marsh Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Cloud Temples Unlock", self.player)
                    or not state.has("Orb", self.player, 15)
                ):
                    return 0
                gems = 375
                if can_headbash(self, state):
                ...
                if not is_boss_defeated(self, "Gulp", state) or not (
                    state.has("Moneybags Unlock - Canyon Speedway Portal", self.player)
                    or is_boss_defeated(self, "Ripto", state)
                ):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Canyon Speedway Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if not is_boss_defeated(
                    self, "Gulp", state
                ) or not can_reach_winter_second_half(self, state):
                    return 0
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Robotica Farms Unlock", self.player)
                ):
                    return 0
                return 400
                ...
                if (
                    self.options.level_lock_options.value == LevelLockOptions.KEYS
                    and not state.has("Metropolis Unlock", self.player)
                    or not state.has("Orb", self.player, 25)
                ):
                    return 0
                return 400
            return 0
        ...
        def has_total_accessible_gems(self, state, max_gems):
            ...
            if not is_boss_defeated(self, "Ripto", state):
                ...
                if (
                    self.options.moneybags_settings == MoneybagsOptions.VANILLA
                    and self.options.enable_gemsanity.value == GemsanityOptions.OFF
                ):
        ...
        def has_sparx_health(self, health, state):
            ...
            max_health = 0
            if (
                self.options.enable_progressive_sparx_health.value
                == SparxUpgradeOptions.BLUE
            ):
                max_health = 2
            elif (
                self.options.enable_progressive_sparx_health.value
                == SparxUpgradeOptions.GREEN
            ):
                max_health = 1
        ...
        if self.options.goal.value == GoalOptions.RIPTO:
            self.multiworld.completion_condition[self.player] = (
                lambda state: is_boss_defeated(self, "Ripto", state)
                and state.has("Orb", self.player, self.options.ripto_door_orbs.value)
            )
        elif self.options.goal.value == GoalOptions.SIXTY_FOUR_ORB:
            self.multiworld.completion_condition[self.player] = (
                lambda state: is_boss_defeated(self, "Ripto", state)
                and state.has("Orb", self.player, 64)
            )
        elif (
            self.options.goal.value == GoalOptions.HUNDRED_PERCENT
            and not self.options.enable_open_world.value
        ):
            self.multiworld.completion_condition[self.player] = (
                lambda state: is_boss_defeated(self, "Ripto", state)
                and state.has("Summer Forest Talisman", self.player, 6)
                and state.has("Autumn Plains Talisman", self.player, 8)
                and state.has("Orb", self.player, 64)
                and has_total_accessible_gems(self, state, 10000)
            )
        elif (
            self.options.goal.value == GoalOptions.HUNDRED_PERCENT
            and self.options.enable_open_world.value
        ):
            self.multiworld.completion_condition[self.player] = (
                lambda state: is_boss_defeated(self, "Ripto", state)
                and state.has("Orb", self.player, 64)
            ...
            self.multiworld.completion_condition[self.player] = (
                lambda state: state.has("Dragon Shores Token", self.player, 10)
                and state.has("Orb", self.player, 55)
                and has_total_accessible_gems(self, state, 8000)
            )
        elif self.options.goal.value == GoalOptions.ALL_SKILLPOINTS:
            self.multiworld.completion_condition[self.player] = lambda state: state.has(
                "Skill Point", self.player, 16
            )
        elif self.options.goal.value == GoalOptions.EPILOGUE:
            self.multiworld.completion_condition[self.player] = (
                lambda state: is_boss_defeated(self, "Ripto", state)
                and state.has("Skill Point", self.player, 16)
            )

        # Summer Forest Rules
        set_rule(
            self.multiworld.get_location(
                "Summer Forest: On a secret ledge", self.player
            ),
        ...
        if Spyro2LocationCategory.LIFE_BOTTLE in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Summer Forest: Life Bottle Near Sunny Beach", self.player
                ),
                lambda state: can_reach_summer_second_half(self, state),
            )
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in swim_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Summer Forest: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Summer Forest: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_swim(self, state),
                    )
            for gem in climb_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Summer Forest: Gem {gem - skipped_bits} requires access to the second half of SF and climb."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Summer Forest: Gem {gem - skipped_bits}"
            ...
            for gem in aquaria_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Summer Forest: Gem {gem - skipped_bits} requires access to the second half of SF and access to beyond the Aquaria wall."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Summer Forest: Gem {gem - skipped_bits}"
        ...
        set_rule(
            self.multiworld.get_location(
                "Glimmer: Gem Lamp Flight in cave", self.player
            ),
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in climb_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Glimmer: Gem {gem - skipped_bits}" in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Glimmer: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_climb(self, state),
                    )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Idol Springs",
                lambda state: state.has("Idol Springs Unlock", self.player),
            )
        set_rule(
            self.multiworld.get_location(
                "Idol Springs: Foreman Bud's puzzles", self.player
            ),
            lambda state: can_swim(self, state),
        )
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in swim_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Idol Springs: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Idol Springs: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_swim(self, state),
                    )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Colossus",
                lambda state: state.has("Colossus Unlock", self.player),
            )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                ...
                "Hurricos",
                lambda state: can_reach_summer_second_half(self, state)
                and state.has("Hurricos Unlock", self.player),
            )
        else:
            set_indirect_rule(
                self,
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            if self.options.enable_progressive_sparx_logic.value:
                set_indirect_rule(
                    self,
                    "Aquaria Towers",
                    lambda state: state.has("Aquaria Towers Unlock", self.player)
                    and can_reach_summer_second_half(self, state)
                    and (
                        state.has(
                            "Moneybags Unlock - Door to Aquaria Towers", self.player
                        )
                    ...
                    and has_sparx_health(self, 1, state),
                )
            else:
                set_indirect_rule(
                    self,
        ...
        set_rule(
            self.multiworld.get_location(
                "Aquaria Towers: Seahorse Rescue", self.player
            ),
            lambda state: state.has(
                "Moneybags Unlock - Aquaria Towers Submarine", self.player
            )
        ...
        if Spyro2LocationCategory.SKILLPOINT in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Aquaria Towers: All Seaweed (Skill Point)", self.player
                ),
                lambda state: state.has(
                    "Moneybags Unlock - Aquaria Towers Submarine", self.player
                )
        ...
        if Spyro2LocationCategory.SKILLPOINT_GOAL in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Aquaria Towers: All Seaweed (Goal)", self.player
                ),
                lambda state: state.has(
                    "Moneybags Unlock - Aquaria Towers Submarine", self.player
                )
        ...
        if Spyro2LocationCategory.SPIRIT_PARTICLE in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Aquaria Towers: All Spirit Particles", self.player
                ),
                lambda state: state.has(
                    "Moneybags Unlock - Aquaria Towers Submarine", self.player
                )
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in sub_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Aquaria Towers: Gem {gem - skipped_bits} requires access to the Aquaria submarine."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Aquaria Towers: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Aquaria Towers: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: state.has(
                            "Moneybags Unlock - Aquaria Towers Submarine", self.player
                        )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Sunny Beach",
                lambda state: state.has("Sunny Beach Unlock", self.player)
                and can_reach_summer_second_half(self, state),
            )
        else:
            set_indirect_rule(
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in climb_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Sunny Beach: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Sunny Beach: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_climb(self, state),
                    )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Ocean Speedway",
                lambda state: state.has("Ocean Speedway Unlock", self.player)
                and can_reach_summer_second_half(self, state)
                and state.has("Orb", self.player, 3),
            )
        else:
            set_indirect_rule(
                self,
        ...
        if not (
            ...
            set_indirect_rule(
                self,
                "Autumn Plains",
                lambda state: is_boss_defeated(self, "Crush", state),
            )
        set_rule(
            self.multiworld.get_location(
                "Autumn Plains: The end of the wall", self.player
            ),
            lambda state: can_reach_metro(self, state)
            or can_pass_autumn_door(self, state),
        )
        set_rule(
        ...
        if Spyro2LocationCategory.MONEYBAGS in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Autumn Plains: Moneybags Unlock: Shady Oasis Portal", self.player
                ),
                lambda state: can_pass_autumn_door(self, state),
            )
        if Spyro2LocationCategory.MONEYBAGS in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Autumn Plains: Moneybags Unlock: Icy Speedway Portal", self.player
                ),
                lambda state: can_pass_autumn_door(self, state),
            )
        if Spyro2LocationCategory.LIFE_BOTTLE in self.enabled_location_categories:
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in whirlwind_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Autumn Plains: Gem {gem - skipped_bits} requires access to Metro Speedway."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Autumn Plains: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Autumn Plains: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_reach_metro(self, state),
                    )
            for gem in climb_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Autumn Plains: Gem {gem - skipped_bits} requires access to the second half of AP."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Autumn Plains: Gem {gem - skipped_bits}"
            ...
            for gem in door_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Autumn Plains: Gem {gem - skipped_bits} requires access to beyond the Professor's door."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Autumn Plains: Gem {gem - skipped_bits}"
            ...
            for gem in shady_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Autumn Plains: Gem {gem - skipped_bits} requires access to the Shady Oasis section."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Autumn Plains: Gem {gem - skipped_bits}"
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            if self.options.enable_progressive_sparx_logic.value:
                set_indirect_rule(
                    ...
                    and has_sparx_health(self, 2, state),
                )
            else:
                set_indirect_rule(
                    self,
        ...
        set_rule(
            self.multiworld.get_location(
                "Crystal Glacier: George the snow leopard", self.player
            ),
            lambda state: state.has(
                "Moneybags Unlock - Crystal Glacier Bridge", self.player
            )
        ...
        if Spyro2LocationCategory.LIFE_BOTTLE in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Crystal Glacier: Life Bottle", self.player
                ),
                lambda state: state.has(
                    "Moneybags Unlock - Crystal Glacier Bridge", self.player
                )
        ...
        if Spyro2LocationCategory.SPIRIT_PARTICLE in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Crystal Glacier: All Spirit Particles", self.player
                ),
                lambda state: state.has(
                    "Moneybags Unlock - Crystal Glacier Bridge", self.player
                )
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in bridge_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Crystal Glacier: Gem {gem - skipped_bits} requires access to the area past the paid bridge."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Crystal Glacier: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Crystal Glacier: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: state.has(
                            "Moneybags Unlock - Crystal Glacier Bridge", self.player
                        )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Zephyr",
                lambda state: state.has("Zephyr Unlock", self.player)
                and (
                    state.has("Moneybags Unlock - Zephyr Portal", self.player)
                    or is_boss_defeated(self, "Ripto", state)
                ),
        ...
        if Spyro2LocationCategory.SPIRIT_PARTICLE in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Zephyr: All Spirit Particles", self.player
                ),
                lambda state: can_climb(self, state),
            )
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in climb_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Zephyr: Gem {gem - skipped_bits}" in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Zephyr: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_climb(self, state),
                    )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Metro Speedway",
                lambda state: state.has("Metro Speedway Unlock", self.player)
                and can_reach_metro(self, state),
            )
        else:
            set_indirect_rule(
                self, "Metro Speedway", lambda state: can_reach_metro(self, state)
            )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Scorch",
                lambda state: state.has("Scorch Unlock", self.player)
                and can_reach_autumn_second_half(self, state),
            )
        else:
            set_indirect_rule(
                self, "Scorch", lambda state: can_reach_autumn_second_half(self, state)
            )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Shady Oasis",
                lambda state: state.has("Shady Oasis Unlock", self.player)
                and can_pass_autumn_door(self, state)
                and (
                    state.has("Moneybags Unlock - Shady Oasis Portal", self.player)
                    or is_boss_defeated(self, "Ripto", state)
                ),
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in headbash_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Shady Oasis: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Shady Oasis: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_headbash(self, state),
                    )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Magma Cone",
                lambda state: state.has("Magma Cone Unlock", self.player)
                and can_pass_autumn_door(self, state),
            )
        else:
            set_indirect_rule(
                self, "Magma Cone", lambda state: can_pass_autumn_door(self, state)
            )
        set_rule(
        ...
        if Spyro2LocationCategory.SPIRIT_PARTICLE in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Magma Cone: All Spirit Particles", self.player
                ),
                lambda state: state.has(
                    "Moneybags Unlock - Magma Cone Elevator", self.player
                )
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in elevator_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Magma Cone: Gem {gem - skipped_bits} requires access past the elevator."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Magma Cone: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Magma Cone: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: state.has(
                            "Moneybags Unlock - Magma Cone Elevator", self.player
                        )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Fracture Hills",
                lambda state: state.has("Fracture Hills Unlock", self.player)
                and can_reach_autumn_second_half(self, state),
            )
        else:
            set_indirect_rule(
        ...
        set_rule(
            self.multiworld.get_location(
                "Fracture Hills: Earthshaper bash", self.player
            ),
            lambda state: can_headbash(self, state),
        )
        if (
            Spyro2LocationCategory.SPIRIT_PARTICLE in self.enabled_location_categories
            and not self.options.fracture_easy_earthshapers
        ):
            set_rule(
                self.multiworld.get_location(
                    "Fracture Hills: All Spirit Particles", self.player
                ),
                lambda state: can_headbash(self, state),
            )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Icy Speedway",
                lambda state: state.has("Icy Speedway Unlock", self.player)
                and can_pass_autumn_door(self, state)
                and (
                    state.has("Moneybags Unlock - Icy Speedway Portal", self.player)
                    or is_boss_defeated(self, "Ripto", state)
        ...
        if not (
            ...
            set_indirect_rule(
                self,
                "Winter Tundra",
                lambda state: is_boss_defeated(self, "Gulp", state),
            )
        set_rule(
            self.multiworld.get_location(
                "Winter Tundra: On the tall wall", self.player
            ),
            lambda state: can_reach_winter_second_half(self, state),
        )
        set_rule(
            self.multiworld.get_location(
                "Winter Tundra: Top of the waterfall", self.player
            ),
            lambda state: can_reach_winter_second_half(self, state),
        )
        set_rule(
        ...
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            ...
            for gem in headbash_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Winter Tundra: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Winter Tundra: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_headbash(self, state),
                    )
            for gem in door_gems:
                ...
                if self.PRINT_GEM_REQS:
                    print(
                        f"Winter Tundra: Gem {gem - skipped_bits} requires access past the Ripto door."
                    )
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Winter Tundra: Gem {gem - skipped_bits}"
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Cloud Temples",
                lambda state: state.has("Cloud Temples Unlock", self.player)
                and state.has("Orb", self.player, 15),
            )
        else:
            set_indirect_rule(
                self, "Cloud Temples", lambda state: state.has("Orb", self.player, 15)
            )
        if Spyro2LocationCategory.GEM in self.enabled_location_categories:
            # Bits of the gems, not accounting for empty bits
            ...
            for gem in headbash_gems:
                ...
                if (
                    len(self.chosen_gem_locations) == 0
                    or f"Cloud Temples: Gem {gem - skipped_bits}"
                    in self.chosen_gem_locations
                ):
                    set_rule(
                        self.multiworld.get_location(
                            f"Cloud Temples: Gem {gem - skipped_bits}", self.player
                        ),
                        lambda state: can_headbash(self, state),
                    )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Canyon Speedway",
                lambda state: state.has("Canyon Speedway Unlock", self.player)
                and (
                    state.has("Moneybags Unlock - Canyon Speedway Portal", self.player)
                    or is_boss_defeated(self, "Ripto", state)
                ),
            ...
            set_indirect_rule(
                ...
                "Canyon Speedway",
                lambda state: state.has(
                    "Moneybags Unlock - Canyon Speedway Portal", self.player
                )
                or is_boss_defeated(self, "Ripto", state),
            )
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Robotica Farms",
                lambda state: state.has("Robotica Farms Unlock", self.player)
                and can_reach_winter_second_half(self, state),
            )
        else:
            set_indirect_rule(
                self,
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Metropolis",
                lambda state: state.has("Metropolis Unlock", self.player)
                and can_reach_winter_second_half(self, state)
                and state.has("Orb", self.player, 25),
            )
        else:
            set_indirect_rule(
                self,
        ...
        if self.options.enable_progressive_sparx_logic.value:
            set_indirect_rule(
                ...
                lambda state: can_reach_winter_second_half(self, state)
                and state.has("Orb", self.player, self.options.ripto_door_orbs.value)
                and has_sparx_health(self, 3, state),
            )
        else:
            set_indirect_rule(
                self,
        ...
        if self.options.level_lock_options.value == LevelLockOptions.KEYS:
            set_indirect_rule(
                self,
                "Dragon Shores",
                lambda state: state.has("Dragon Shores Unlock", self.player)
                and is_boss_defeated(self, "Ripto", state),
            )
        else:
            set_indirect_rule(
                self,
        ...
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Tunnel o' Love", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Shooting Gallery I", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Shooting Gallery II", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Shooting Gallery III", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Rollercoaster I", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Rollercoaster II", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Rollercoaster III", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        ...
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Dunk Tank II", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        if Spyro2LocationCategory.SHORES_TOKEN in self.enabled_location_categories:
            set_rule(
                self.multiworld.get_location(
                    "Dragon Shores: Dunk Tank III", self.player
                ),
                lambda state: has_total_accessible_gems(self, state, 8000)
                and state.has("Orb", self.player, 55),
            )
        ...
        for level in self.all_levels:
            ...
            if Spyro2LocationCategory.GEM_25 in self.enabled_location_categories:
                set_rule(
                    ...
                    lambda state, level=level: get_gems_accessible_in_level(
                        self, level, state
                    )
                    >= 100,
                )
        ...
        if Spyro2LocationCategory.TOTAL_GEM in self.enabled_location_categories:
            for i in range(20):
                ...
                if gems <= self.options.max_total_gem_checks.value:
                    set_rule(
                        self.multiworld.get_location(
                            f"Total Gems: {gems}", self.player
                        ),
                        lambda state, gems=gems: has_total_accessible_gems(
                            self, state, gems
                        ),
                    )
                else:
    ...
    def fill_slot_data(self) -> Dict[str, object]:
        ...
        for location in self.multiworld.get_filled_locations():
            ...
            if location.player == self.player:
                ...
                locations_address.append(
                    item_dictionary[
                        location_dictionary[location.name].default_item
                    ].s2_code
                )
